...
实现了纯 2D 源地址和目的地址范围校验，使用 64 位中间计算避免 32 位中间回绕，并拒绝零尺寸、`stride_pixels < width_pixels`、地址超出 32 位范围以及超过 `sdram_limit` 的传输。
...
审查确认原计划中的目的地址样例按完整 stride 公式仍低于 `0x01000000`，不能作为失败用例，因此保留为正向断言。目的地址测试现覆盖：

- 基于 `0x00F70000` 的真实越界场景，结束地址大于 `sdram_limit`。
- 基地址 `0x00FFFF00`、单行 128 像素传输，结束地址恰好等于 `0x01000000`，应通过。

报告现准确描述为：64-bit 中间计算避免 32-bit 中间回绕。
