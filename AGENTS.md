...
- 键盘以 `docs/Netlist_Schematic1_2026-09-08.tel`（确定性网表）为准：`PB0..PB3`=`KEY_ROW1..4`（输出，空闲低、逐行拉高）、`PB4..PB11`=`KEY_COL1..8`（输入，`R9..R16` 33k 外部下拉到地，逻辑列 col0=`PB11` … col7=`PB4` 反序）；行列开关为 `SW1..SW8` / `SW11..SW16` / `SW17..SW24` / `SW26..SW32`；`PC13/PC14/PC15` 空出。旧 `PA0..PA3` 行选命名与早期的上拉假设已作废——列线开内部上拉会与下拉分压出约 1.5V（不定态），必须用无上下拉输入。
...
- **V16 链路参数（2026-09-08 HEX 逆向定案）**：串口 **9600 8N1 无流控**——上游按“名义 38400”算 BRR，但其常数组合（`BRR=(25*fclk/baud/100)<<4|frac`）实际落在 fclk/4×名义值，72MHz 下 BRR=0x1D4C=精确 9600。**握手**：主机发单字节 `0x0F`，面板立即回 12 字节身份串 `"2000 V16  \x80\x00"`（32 30 30 30 20 56 31 36 20 20 80 00，位于 V16 .data `0x080057B8`→RAM `0x2000001C`，USART1 IRQ 内直发）；主机蜂鸣/开始发显示帧以此为条件。面板 TX 其余只有按键码，无任何文本。本固件已在 **RX 中断内拦截 `0x0F` 即回身份串**（与 V16 同为 IRQ 直发——主机查询超时短于固件 boot，主循环应答会错过窗口；教训 2026-09-08），主循环只丢弃 `0x0F` 不喂解析器，波特率同 9600。
- **主机显示流实测定案（2026-09-08 总线抓包）**：主机每帧 = 五连状态字节 `[0A][0E][08][07][09]+值` + 自由文本（无 `0x04` POS）。读数带前导空格与槽位计数（`"0 0.011014 VDC"`、`"-030.4414mVDC."` 尾部光标点）；开路输入时 AUTO 量程猎动，各量程占位符（`--.----- AAC/ADC/VDC/MOHM`）与 `NEW CODE?`（含 `0B 01'N'0B 00` 闪标）轮播——**这是主机正常行为，不是幻按键**（KEYDIAG 审计证明面板零幻码）。`reading_split` 因此改为**从右往左解析**：尾部字母跑=单位、倒数第一词=数字、无数字即空（占位符/标签被读数过滤器丢弃）。RIF 字符集未命中=跳字模不自毁；按键路径有上电 2s 静默+同码 300ms 节流两道防幽灵栅栏（KEYDIAG 1Hz 审计行仅 UART_LOG=1 编入）。
- 任何新固件先做离线构建和静态检查，再在限流电源、断开仪器高压测量路径的条件下验证；首次烧录保留原始 V15/V16 镜像和可恢复的 SWD 接线。
- **开机花屏定案（2026-09-10）**：开机左上角绿色残缺字迹、右下角彩条滚动闪烁 = 上电到消隐之间的**失控窗口**。PA3（共享复位线）在 `init_gpio` 里一上来就抬高水平，LT7680 自上电起就跑默认寄存器态（显示开 + 彩条测试图案、默认时序）直扫背光面板；热复位时 SDRAM 还保留上一帧，绿色大数字/徽标残片先漏出来。`K2000_UART_LOG=1` 时 9600bd 横幅把这窗口拉长到数百毫秒，每次上电必现。修复：`hal_display_early_reset_hold()` 作为 `main()` 第一句（HAL_Init 前）把 PA3 按低，`init_gpio` 里 LCM_RES 默认保持低；`hal_display_boot_blank()` 复位脉冲后从 +10ms 起每 5ms 尝试 `REG[12h]=0x08` 写+回读校验（上限 100ms），确认后补足 60ms settle 再走 read_status/panel init；wait_ready 后的二次消隐保留作保险。见 skill lt7680-st7701「Boot sequence cosmetics」。
...
- **共享线上任何非协议字节都会导致主机误识别到按键（2026-09-10 定案）**：显示总线是半双工共享线（BSS138 电平移位是双向的，主机发数字节时面板侧网络上"听得见"，反之亦然），主机把线上收到的任何不认识字节都往按键/解析器状态机里喂。实证事故：boot 期 STALL 诊断的 `perf_send_u32()` 漏了 UART_LOG 门控，`[STALL] gap=3519 tick=3519` 的文本标签被 stub 丢弃但**两个裸数字上线**，主机端立即出现按键换挡行为——早已误判为"幻按键/EMI"数日。因此面板 TX 的铁律：**只允许两种输出**——①一次性身份串 `"2000 V16  \x80\x00"`（对 0x0F）②单字节按键码 0x40~0x60。任何新增发送（诊断/日志/hex 转储）必须编译期门控（`perf_send_u32` 已随 `perf_send_u32` 整体纳入 UART_LOG 门控并从二进制消失，反汇编审计 `hal_uart_send` 直呼点只剩按键转发与 PERF 字符串自检且后者运行时被 PERF_LOG=0 封死）。验收手段：**孤立静默测试**——拔掉主机、单独复位面板，TX 必须零字节（无 0x0F 连身份串都不发）。
...
- **开机"极卡"定案（2026-09-11 计数器取证）**：开路输入下主机上电后以 ~27 样本/秒轮播全部档位数分钟（其自身行为），单位门锁定单位的样本每轮只出现一次 → 有效供给 ~0.6/s → 提交 1.1/s（健康 14.6/s）= 用户感知的卡顿。显示初始化/构建开销/占位符渲染假设全部计数器归零排除。这不是面板 bug：主机稳定为双单位振荡后（drop 53%、pass 15/s）即流畅。取舍：保持锁定（读数自洽、轮播期 ~1/s 更新）vs 透传（忠实但 27/s 换代必撕裂）。接真实信号轮播不发生。取证工具：`tools/swd_gate_census.sh`（窗口速率）、`tools/swd_boot_census.sh`（开机三窗口自动画像）。
- **单位抖动把 PRESENT 提交压成活锁 = 连主机黑屏（2026-09-11 SWD 取证定案）**：主机 AUTO 量程猎动让单位身份以 ~1Hz 翻转，而趋势背景重绘一次需要 >1s，三处代码在每次翻转时把排空进度归零：①`reading_only_invalidate_trend_pages()` 清 scroll/column/bg_valid；②`reading_only_render_trend_background()` 里 `pending_unit != unit` 就 `idx=0`；③排空永远完不成 → `background_ready` 永假 → 事务永不解除 → PRESENT 进不去 → 提交活锁 → boot 消隐让面板保持全黑。**期限检查放 PRESENT 入口等于没有**（PRESENT 恰是到不了的地方）。修复：事务期间 invalidate 只排队 pending；drain 只按"页面缓存单位"变化重启；期限移到 TREND 入口。SWD 驻留计数取证数据：修复前 TREND 进入 2795 次 / PRESENT 1 次 / 期限触发 0；修复后 PRESENT 557 次 / 期限触发 6 次。**渲染管线在 CubeMX 树无宿主测试镜像——`tools/swd_display_loop.sh`（单调提交计数 + 阶段驻留普查 + abort/期限计数，AIRCR 复位）是此类 bug 的常备回归手段，红=零提交=必黑屏**。Demo 模式单位轮换慢（2-3s）永远复现不了——连主机问题必须连主机复现。
- **SWD 活体取证方法（2026-09-10 验证有效）**：无串口时用 OpenOCD 直接定位"黑屏但程序在跑"类问题——①`nm` 找关键静态变量地址，`halt`+`mdw/mdb` 采样状态（phase/stage/dirty/错误码/画布内容），隔秒两次对比判断"冻结"还是"慢速"；②对存疑函数设硬件断点（`bp <addr> 2`）+`sleep` 后查 `reg pc`，断点不命中=该函数从未被调用（注意地址抄错一个 nibble 就是假阴性）；③`wp <addr> 4 w` 观察点抓"谁在写这个变量"；④密集采样 `reg pc` 看热点分布（SPI 轮询环 vs 主循环）。反汇编假阴性陷阱：PERF_LOG=0 时窗口计数器**每个主循环都清零**，采样读数恒 0 不代表事件没发生；变量被 LTO 内联消失（如 `s_render_item` 无符号）时改从 WDT 备份寄存器或加诊断打印。
