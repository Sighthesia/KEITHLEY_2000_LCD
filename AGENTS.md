...
- **V16 链路参数（2026-09-08 HEX 逆向定案）**：串口 **9600 8N1 无流控**——上游按“名义 38400”算 BRR，但其常数组合（`BRR=(25*fclk/baud/100)<<4|frac`）实际落在 fclk/4×名义值，72MHz 下 BRR=0x1D4C=精确 9600。**握手**：主机发单字节 `0x0F`，面板立即回 12 字节身份串 `"2000 V16  \x80\x00"`（32 30 30 30 20 56 31 36 20 20 80 00，位于 V16 .data `0x080057B8`→RAM `0x2000001C`，USART1 IRQ 内直发）；主机蜂鸣/开始发显示帧以此为条件。面板 TX 其余只有按键码，无任何文本。本固件已在主循环拦截 `0x0F` 回身份串（不喂解析器），波特率同 9600。
