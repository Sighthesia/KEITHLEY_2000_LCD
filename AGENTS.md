...
- **单位抖动把 PRESENT 提交压成活锁 = 连主机黑屏（2026-09-10 定案）**：`s_trend_rebuild_transaction` 的本意是"趋势换单位时压住翻页等隐藏页画完"，但主机 AUTO 量程猎动让单位身份以 ~1Hz 翻转，每次翻转重置排空进度，PRESENT 分支 `stage=READING_ONLY_TREND; return;` 把提交弹回 TREND **永远到不了 `lt7680_gfx_present_page`** → 面板停在 boot 黑页。现场特征极具迷惑性：字段计数/画布/主循环（1-13ms）全部健康，只有提交为零。修复：①单次事务 hold 上限 700ms，到期强制提交（半重建趋势好过黑屏）；②排空中到达的二次翻转合并为排队重建，不再原地重启。Demo 模式单位轮换慢（2-3s）永远复现不了——**连主机问题必须连主机复现**。
