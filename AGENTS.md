...
- 趋势坐标轴单元格（2026-08-16 增补）：趋势区 Y/X 轴采用与读数区信息格同款的深灰单元格（`MAIN_DISPLAY_COLOR_BAR`=#182126），L 形围框——Y 轴占 x0..95 整条竖带（y192..319 全高），X 轴占底部横带（x96..935，y296..319）；Y 轴标签在单元格内**右对齐**（距绘图区左缘 4px，超长 clamp 到 x=0），X 轴标签保持居中。绘图区网格、曲线、坐标文字颜色不变。仿真器 `sim/index.html` 的 `L.yAxis*`/`L.xAxis*` 与固件 `main_display.h` 的 `MAIN_DISPLAY_Y_AXIS_*`/`MAIN_DISPLAY_X_AXIS_*` 常量一一对应，`verify.js` 断言轴单元格围住绘图区。
