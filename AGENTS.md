...
- **喂样率要求反转（2026-09-04）**：bench 现要求 500Hz 输入 sustained＋30fps 显示，不再回避速率。`K2000_DEMO_INPUT_HZ=500` 配 catch-up 预算 64；管线为此硬化（sweep 回读探针默认关、文本单页画＋BTE 条带同步、header 字形差分、TREND 读数让行）。判据：`input_hz≈500、missed=0`；`fps` 目前 13~19（逐操作切页＋引擎忙等待所限，30fps 需驱动层批量优化）。旧"回 10Hz"结论作废。
