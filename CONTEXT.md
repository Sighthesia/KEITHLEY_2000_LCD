...
- **静态装饰 (Static decoration)**: 读数页中不随数据变化的固定视觉元素（状态栏下缘与单位行下缘分隔线）。随每次渲染重绘以抵抗背景清黑，视觉上始终存在、不随数据变化。
- **无数据态 (No-data state)**: 启动期尚未收到任何主机消息时显示板的等待状态；读数区显示无数据提示。与主机下发的特殊读数 `----` 不同——后者是主机权威内容，前者是显示板本地状态。收到任意主机消息即退出。
- **界面突变 (UI jump)**: 首次渲染时基础框架整体出现/消失或布局跳变造成的视觉突兀；通过先绘制静态装饰、无数据态仅显示提示文本来避免。
