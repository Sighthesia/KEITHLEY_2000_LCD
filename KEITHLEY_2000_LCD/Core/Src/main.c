    ...
    while (1)
    {
#if !K2000_DEMO_FEED
        {
            ...
            while (rx_budget-- > 0u && (ch = hal_uart_receive_byte()) >= 0)
            {
                 ...
                 * single 0x0F; the identity string is already answered in
                 * the RX IRQ (hal_board.c). Drop 0x0F here so the parser
                 * never sees it (it would open a bogus field tag). */
                if (ch == 0x0F)
                {
                    continue;
                }
