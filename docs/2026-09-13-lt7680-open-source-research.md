...
范围校验由固件测试和传输范围测试覆盖；RIF cache cleanup 的离线 mock 测试也已通过。这些测试不等于 LT7680 实机状态恢复已验证，Flash/BTE/GE 的完整状态恢复仍是未决。开源 RA8876 资料只能提供交叉检查，不能替代目标 LT7680A-R 和本板的固定图案、长时间压力实测。
