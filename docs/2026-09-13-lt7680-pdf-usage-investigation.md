...
2026-09-14 已完成最终同步与验证。`rif_tile_cache.c` 两棵源码树完全一致；`lt7680_gfx.c` 的差异属于 CubeMX 硬件适配差异，不能据此判定共享逻辑失同步。范围检查和 cache 失败恢复已实现并通过宿主测试，覆盖完整跨行范围、stride 约束以及失败后的 Canvas 状态恢复。
