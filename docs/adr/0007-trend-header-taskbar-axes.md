...
## 作业续画＋擦除越界（2026-09-04 实测驱动）

- 让行在字形作业半程中断，恢复链上其它绘制会续画旧串（错字＋缓存投毒）。修复：让行加双作业 idle 门卫。
- 差分擦除 26px 高出内容带 2px，青色糊上绘图区顶边。修复：擦除限 24px 内容带。
- 实测：fps 24~28、帧 9~14ms、input 500＋/missed 0；轮换挡跌一帧即恢复。此前一次"串口静默"为 openocd 会话后 tty 参数丢失，非卡死。
