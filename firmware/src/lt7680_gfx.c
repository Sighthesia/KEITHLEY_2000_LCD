...
static lt7680_status_t flash_snapshot_spi_regs(lt7680_flash_spi_snapshot_t *probe)
{
    ...
    st = lt7680_read_reg(REG_SFL_CTRL, &probe->b7);
    if (st != LT7680_OK) {
        probe->status = st;
        return st;
    }
    ...
    st = lt7680_read_reg(REG_SPIMCR2, &probe->b9);
    if (st != LT7680_OK) {
        probe->status = st;
        return st;
    }
    st = lt7680_read_reg(REG_SPIMSR, &probe->ba);
    if (st != LT7680_OK) {
        probe->status = st;
        return st;
    }
    st = lt7680_read_reg(REG_SPI_DIV, &probe->bb);
    if (st != LT7680_OK) {
        probe->status = st;
        return st;
    }
...
static lt7680_status_t spi_push_and_drain_raw(const uint8_t *tx, uint8_t count,
...
static lt7680_status_t spi_push_and_drain(const uint8_t *tx, uint8_t count,
                                          uint8_t discard, uint8_t *data)
{
    return spi_push_and_drain_raw(tx, count, discard, data, 0);
}
...
lt7680_status_t lt7680_flash_read_jedec_id(uint8_t id[3])
{
    ...
    st = spi_begin();
    if (st == LT7680_OK) {
        st = spi_push_and_drain_raw(command, sizeof(command), 1u, id,
